...
<template>
  <div class="group relative inline-block">
    ...
    >
      <div class="flex flex-col overflow-hidden bg-surface rounded-sm shadow-md">
        <button
          v-for="localeItem in locales"
          :key="localeItem.code"
          @click="setLocale(localeItem.code);"
          class="text-left px-4 py-2 text-sm hover:text-primary transition-colors duratio-200"
          :class="{ 'text-primary font-medium': locale === localeItem.code }"
        >
          {{ localeItem.name }}
        </button>
      </div>
